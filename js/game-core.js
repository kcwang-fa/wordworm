/* ============================================================
 * game-core.js —— 全站共用地基（必須最先載入的 game 檔）
 * 內容：字典解析工具、遊戲模式切換、字母池與計分表、
 *       棋盤生成（含母音平衡）、toast/蟲蟲表情、
 *       共用狀態（grid、sel、score、COLS/ROWS、gameMode…）。
 * 為什麼獨立成檔：其他所有 game-*.js 都依賴這裡宣告的全域，
 * 載入順序上它必須排第一，詳見 index.html 的 script 區。
 * ============================================================ */
...
function ensureDictionaryLoaded(options = {}) {
  ...
  dictLoadPromise = Promise.all([
    ...
    const extraSet = new Set(extraWords);
    const extraAdded = [...extraSet].filter(w => !coreSet.has(w)).length;
    DICT = new Set([...coreWords, ...extraWords]);
